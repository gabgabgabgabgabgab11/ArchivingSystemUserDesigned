...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 01, 2025 at 10:34 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteDocument` (IN `p_id` INT)   BEGIN
    DELETE FROM documents WHERE id = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteDocumentType` (IN `p_id` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllDocuments` ()   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDepartmentIdByName` (IN `p_dept_name` VARCHAR(100))   BEGIN
    SELECT id FROM departments WHERE name = p_dept_name;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDocumentById` (IN `p_id` INT)   BEGIN
    SELECT d.*, dt.type_name, dep.name AS department_name
    FROM documents d
    LEFT JOIN document_types dt ON d.type_id = dt.id
    LEFT JOIN departments dep ON d.department_id = dep.id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetDocumentStats` ()   BEGIN
    -- First result set: total documents
    SELECT COUNT(*) AS total FROM documents;
    -- Second result set: count by type
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetRecentActivities` (IN `p_limit` INT)   BEGIN
    SELECT * FROM activity_log ORDER BY timestamp DESC LIMIT p_limit;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetTypeIdByName` (IN `p_type_name` VARCHAR(50))   BEGIN
    SELECT id FROM document_types WHERE type_name = p_type_name;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertDepartment` (IN `p_name` VARCHAR(100))   BEGIN
    INSERT INTO departments (name) VALUES (p_name);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertDocument` (IN `p_title` VARCHAR(255), IN `p_authors` VARCHAR(255), IN `p_type_id` INT, IN `p_description` TEXT, IN `p_year_published` INT, IN `p_file_path` VARCHAR(500), IN `p_date_archived` DATETIME, IN `p_department_id` INT)   BEGIN
    INSERT INTO documents (title, authors, type_id, description, year_published, file_path, date_archived, department_id)
    VALUES (p_title, p_authors, p_type_id, p_description, p_year_published, p_file_path, p_date_archived, p_department_id);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertDocumentType` (IN `p_type_name` VARCHAR(50))   BEGIN
    INSERT INTO document_types (type_name) VALUES (p_type_name);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertUser` (IN `p_username` VARCHAR(50), IN `p_password` VARCHAR(255))   BEGIN
    INSERT INTO users (username, password) VALUES (p_username, p_password);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `LogActivity` (IN `p_timestamp` DATETIME, IN `p_username` VARCHAR(100), IN `p_action` VARCHAR(20), IN `p_document_title` VARCHAR(255), IN `p_document_type` VARCHAR(100), IN `p_details` TEXT)   BEGIN
    INSERT INTO activity_log (timestamp, username, action, document_title, document_type, details)
    VALUES (p_timestamp, p_username, p_action, p_document_title, p_document_type, p_details);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateDepartment` (IN `p_id` INT, IN `p_name` VARCHAR(100))   BEGIN
    UPDATE departments SET name = p_name WHERE id = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateDocument` (IN `p_id` INT, IN `p_title` VARCHAR(255), IN `p_authors` VARCHAR(255), IN `p_type_id` INT, IN `p_description` TEXT, IN `p_year_published` INT, IN `p_file_path` VARCHAR(500), IN `p_date_archived` DATETIME, IN `p_department_id` INT)   BEGIN
    UPDATE documents
    SET title = p_title,
        authors = p_authors,
        type_id = p_type_id,
        ...
        date_archived = p_date_archived,
        department_id = p_department_id
    WHERE id = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateDocumentType` (IN `p_id` INT, IN `p_type_name` VARCHAR(50))   BEGIN
    UPDATE document_types SET type_name = p_type_name WHERE id = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateUserPassword` (IN `p_id` INT, IN `p_password` VARCHAR(255))   BEGIN
...
(29, '2025-07-01 13:56:42', 'The Admin', 'Deleted', 'Updated', 'OTHERS', 'Deleted document'),
(30, '2025-07-01 14:36:19', 'The Admin', 'Added', 'Sample', NULL, 'Added a new document'),
(31, '2025-07-01 14:38:12', 'The Admin', 'Edited', 'Sample Updated', 'OTHERS', 'Edited document details'),
(32, '2025-07-01 14:38:50', 'The Admin', 'Deleted', 'Sample Updated', 'OTHERS', 'Deleted document'),
(33, '2025-07-01 14:39:12', 'The Admin', 'Deleted', 'Sample Document', 'RESEARCH/THESIS', 'Deleted document'),
(34, '2025-07-01 16:24:29', 'The Admin', 'Added', 'sample', NULL, 'Added a new document'),
(35, '2025-07-01 16:25:08', 'The Admin', 'Edited', 'asdasdasda', 'RESEARCH/THESIS', 'Edited document details'),
(36, '2025-07-01 16:25:18', 'The Admin', 'Edited', 'asdasdasda', 'RESEARCH/THESIS', 'Edited document details'),
(37, '2025-07-01 16:25:28', 'The Admin', 'Edited', 'asdasdasda', 'OJT TERMINAL REPORT', 'Edited document details'),
(38, '2025-07-01 16:25:46', 'The Admin', 'Deleted', 'asdasdasda', 'OTHERS', 'Deleted document');
